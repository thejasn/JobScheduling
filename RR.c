...
void processincome(struct process list[],struct process queue[], int ctr, int *listn, int *qn)
{
	int i=0, x=0;
	while(i<(*listn))
	{
...
int isempty(struct process queue[], int qn)
{
	int i=0, flag=0;
	for(;i<qn;i++)
		if(queue[i].BT==0)
...
int GanttChart(struct process list[],struct process queue[])
{
	...
	clock_t start,end;
	start = clock();
	while(listn>0 || !isempty(queue,qn))
	{
		
		processincome(list,queue,ctr,&listn,&qn);
		if(qn>0)
			i=i%qn;
		else{
			ctr+=TQ;
		...
		if(queue[i].BT>0)
		{
			...
			{
				ctr+= queue[i].BT;
				queue[i].TAT = ctr - queue[i].AT;
				queue[i].WT = queue[i].TAT - queue[i].WT;
				queue[i].BT=0;
				totalTAT += queue[i].TAT;
				totalwait += queue[i].WT;
			}
			else{
				queue[i].BT -= TQ;
				ctr+= TQ;
...
{
	struct process queue[100000],list[100000];
	int qn, i=0;
	ps=atoi(argv[1]);
	qn = GanttChart(list,queue);
	
	printf("\n %lf ",totalwait/ps); 
	printf("%lf",totalTAT/ps);
